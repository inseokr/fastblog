...
struct CaptionExperimentView: View {
    ...
    @State private var ranSamples: [PhotoSample] = []   // the samples that produced `rows` (index-aligned)
    ...
    /// Photo + caption pairs from the chosen path, for Travel DNA (so cards can show real photos).
    /// `row.index` is the position within the samples that were actually run (`ranSamples`).
    private var dnaItems: [DNAPhoto] {
        rows.compactMap { row in
            let c = caption(row, dnaSource)
            guard !c.isEmpty else { return nil }
            let image = ranSamples.indices.contains(row.index) ? ranSamples[row.index].image : nil
            return DNAPhoto(image: image, caption: c)
        }
    }
    ...
    private func run() async {
        ...
        ranSamples = toRun   // keep index-aligned with rows so DNA cards can show the right photos
