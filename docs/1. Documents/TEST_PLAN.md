...
### 9.4 Carousel Slides Sharing
| # | Steps | Expected | Priority |
|---|-------|----------|----------|
| SH-5 | Open blog → Share → "Carousel Slides" | Carousel export generation starts, no crash | P0 |
| SH-6 | Carousel generation completes | Slides preview shown with correct photos and layout | P0 |
| SH-7 | Tap share on generated carousel | iOS share sheet appears with slides attached (images or archive) | P0 |
| SH-8 | Carousel with a single-photo place stop | Slide renders correctly, no layout breakage | P1 |
| SH-9 | Carousel with 10+ place stops / slides | All slides generated, no truncation or crash | P1 |
| SH-10 | Cancel carousel export mid-generation | Export cancelled cleanly, no crash or stuck state | P1 |
| SH-11 | Carousel exported to Photos / Save Image | Images saved to photo library, success confirmation shown | P1 |

### 9.5 Reel Generator Sharing
| # | Steps | Expected | Priority |
|---|-------|----------|----------|
| SH-12 | Open blog → Share → "Create Reel" | Reel generation starts, progress indicator shown | P0 |
| SH-13 | Reel generation completes | Reel preview playable in-app, no crash | P0 |
| SH-14 | Tap share on generated reel | iOS share sheet appears with video attached | P0 |
| SH-15 | Reel with fewer than 3 photos | Reel still generates (or clear minimum-photos error shown) | P1 |
| SH-16 | Reel with 50+ photos | Generation completes in reasonable time, no memory crash | P1 |
| SH-17 | Cancel reel generation mid-way | Generation cancelled cleanly, no stuck spinner | P1 |
| SH-18 | Save reel to Photos library | Video saved successfully, confirmation shown | P1 |
| SH-19 | Reel generation while offline | Graceful error if network is required; local generation works if offline-capable | P1 |
| SH-20 | Reel on iOS 16 / 17 device | No crash; falls back or shows unsupported message if feature is gated | P0 |
