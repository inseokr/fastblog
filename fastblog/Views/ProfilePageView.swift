...
struct ProfilePageView: View {
    ...
    @State private var blogToUpload: CreatedRecapBlog? = nil
    ...
    private var searchBar: some View {
        ...
        .alert("Upload to Cloud?", isPresented: $showUploadPromptAlert) {
            ...
            Text("This blog needs to be uploaded to the cloud before you can share a link. Open the blog to upload it?")
        }
    }
                            ...
                            if let key = blog.blogKey,
                               let url = SecureShareToken.shareURL(username: BlogCard.resolvedUsername, blogKey: key) {
                                ...
                                .highPriorityGesture(TapGesture().onEnded {
                                    print("Share button tapped via HighPriorityGesture!")
                                    onShareUnuploaded?()
                                })
                            }
                        }
...
struct StoryFeedSection: View {
    ...
    var onShareUnuploaded: ((CreatedRecapBlog) -> Void)? = nil
    
    var body: some View {
        VStack(spacing: ProfileTheme.Spacing.xxl) {
            ForEach(blogs) { blog in
                BlogCard(blog: blog, onShareUnuploaded: {
                    onShareUnuploaded?(blog)
                })
                    .contentShape(Rectangle())
                    .onTapGesture {
                        isSearchFocused = false
