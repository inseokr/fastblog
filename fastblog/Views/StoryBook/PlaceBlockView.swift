...
struct PlaceBlockView: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(alignment: .center, spacing: 4) {
                ...
                if let url = Self.googleSearchURL(placeName: place.title, placeSubtitle: place.subtitle) {
    ...
    private func captionSection(caption: String, pageWidth: CGFloat) -> some View {
        ...
        let topGap: CGFloat = 6 // matches VStack(spacing:) between title and caption
        let bottomGap: CGFloat = hasPhotos ? 6 : 0 // caption <-> photos spacing only when photos exist
...
/// External-link affordance to the right of story-mode place titles (opens Google search via the wrapping `Link`).
struct StoryPlaceExternalLinkIcon: View {
    let titleFontSize: CGFloat
    let foregroundColor: Color

    private var iconSide: CGFloat {
        max(10, titleFontSize * 0.72)
    }
